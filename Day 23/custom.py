...
class LeCartCustom(LeBasecart):
	...
	def get_categories_ext_export(self, categories):
		...
		categories_ext_queries = {
			...
			'languages': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_languages "
			},
			# 'URIs': {
			# 	'type': 'select',
	...
	def convert_category_export(self, categories, categories_ext):
		category_data = self.construct_category()
		parent = self.construct_category_parent()
		parent['id'] = 0
		if categories['parent_id']:
			parent_data = self.get_category_parent(categories['parent_id'])
			if parent_data['result'] == 'success':
				parent = parent_data['data']
		category_data['id'] = categories['categories_id']
		category_data['parent'] = parent
		# category_data['active'] = True if categories['active']=='1' else False
		img = get_row_from_list_by_field(categories['categories_image'], 'categories_id', categories['categories_id'])
		if img and img['categories_image'] != '':
			category_data['thumb_image']['url'] = 'http://www.iemotorsport.com/mm5/'
			category_data['thumb_image']['path'] = img['categories_image']
		category_data['sort_order'] = categories['sort_order']
		category_data['created_at'] = datetime.fromtimestamp(to_int(categories['date_added'])) if categories['date_added'] else '0000-00-00 00:00:00'
		category_data['updated_at'] = datetime.fromtimestamp(to_int(categories['last_modified'])) if categories['last_modified'] else '0000-00-00 00:00:00'
		# category_description = get_row_from_list_by_field(categories_ext['data']['categories_description'],'categories_id', categories['categories_id'])['categories_name']
		...
		category_data['name'] = get_row_from_list_by_field(categories_ext['data']['categories_description'], 'categories_id', categories['categories_id'])['categories_name']
		# category_data['meta_title'] = categories['page_title']
		# category_data['meta_keyword'] = categories['code']
		category_data['code'] = get_row_from_list_by_field(categories_ext['data']['languages'], 'languages_id', '1')['code']
		# meta_description
		...
		if self._notice['config']['seo_301']:
			...
			category_data['seo'] = getattr(self, 'categories_' + detect_seo)(categories, categories_ext)
		self.log(category_data, 'categories_data')
		return response_success(category_data)

	def get_category_id_import(self, convert, categories, categories_ext):
		return categories['categories_id']

	def check_category_import(self, convert, categories, categories_ext):
		id_imported = self.get_map_field_by_src(self.TYPE_CATEGORY, convert['id'], convert['code'])
		return id_imported

	def router_category_import(self, convert, categories, categories_ext):
		return response_success('category_import')

	def before_category_import(self, convert, categories, categories_ext):
		return response_success()

	def category_import(self, convert, categories, categories_ext):
		return response_success(0)

	def after_category_import(self, category_id, categconvertory_data, categories, categories_ext):
		return response_success()

	def addition_category_import(self, convert, categories, categories_ext):
		return response_success()
	...
	def get_products_ext_export(self, products):
		...
		if (not product_ext) or product_ext['result'] != 'success':
			return response_error()
		image_ids = duplicate_field_value_from_list(products['data'], 'products_images')
	...
	def convert_product_export(self, products, product_ext):
		products_ext_data = product_ext['data']
		product_data = self.construct_product()
		product_data['id'] = products['products_id']
		product_data['sku'] = products['products_upc_code']
		product_data['price'] = products['products_msrp']
		product_data['special_price']['price'] = products['products_wholesale_price']
		product_data['weight'] = products['products_weight']
		# if to_int(products['active']) > 0:
		# 	status = True
		# else:
		# 	status = False
		...
		product_data['created_at'] = datetime.fromtimestamp(to_int(products['products_date_added'])) if products['products_date_added'] else get_current_time()
		product_data['updated_at'] = datetime.fromtimestamp(to_int(products['products_last_modified'])) if to_int(products['products_last_modified']) else product_data['created_at']
		product_data['name'] = get_row_from_list_by_field(product_ext['data']['products_description'],'products_id', products['products_id'])['products_name']
		product_data['description'] = get_row_from_list_by_field(product_ext['data']['products_description'],'products_id', products['products_id'])['products_description']
		product_description = get_list_from_list_by_field(products_ext_data['product_to_category'], 'products_id',products['products_id'])
		...
		# 	product_data['thumb_image']['path'] = products['products_image']


		# product_images = get_list_from_list_by_field(products_ext_data['products_images'], 'product_id',products['id'])
		# if product_images:
		# 	if check_thumbnail:
		...
		# special = get_row_from_list_by_field(products_ext_data['specials'], 'products_id', products['products_id'])
		# if special:
		...
		if self._notice['config']['seo_301']:
			detect_seo = self.detect_seo()
			product_data['seo'] = getattr(self, 'products_' + detect_seo)(products, product_ext)
		return response_success(product_data)

	def get_product_id_import(self, convert, products, product_ext):
		return products['products_id']

	def check_product_import(self, convert, products, product_ext):
		return True if self.get_map_field_by_src(self.TYPE_PRODUCT, convert['id'], convert['code']) else False

	def router_product_import(self, convert, products, product_ext):
		return response_success('product_import')

	def before_product_import(self, convert, products, product_ext):
		return response_success()

	def product_import(self, convert, products, product_ext):
		return response_success(0)

	def after_product_import(self, products_id, convert, products, product_ext):
		return response_success()

	def addition_product_import(self, convert, products, product_ext):
		return response_success()
	...
	def prepare_customers_import(self):
		query = {
			'type': 'query',
			'query': "ALTER TABLE _DBPRF_customers MODIFY COLUMN customsers_password varchar(255)"
		}
	...
	def convert_customer_export(self, customers, customers_ext):
		...
		# customer_data['username'] = customers['login']
		...
		customer_data['dob'] = customers['customers_dob']
		...
		if address_books:
			for address_book in address_books:
				...
				# address_data['telephone'] = customers['bill_phone']
				# address_data['company'] = address_book['comp']
				# address_data['fax'] = customers['bill_fax']
				...
				if country:
					...
					address_data['country']['country_code'] = country['countries_iso_code_3']
				...
				if state_id:
					state = get_row_from_list_by_field(customers_ext['data']['zones'], 'zone_id', address_book['entry_zone_id'])
					if state:
	...
	def get_customer_id_import(self, convert, customers, customers_ext):
		return customers['customers_id']

	def check_customer_import(self, convert, customers, customers_ext):
		return True if self.get_map_field_by_src(self.TYPE_CUSTOMER, convert['id'], convert['code']) else False

	def router_customer_import(self, convert, customers, customers_ext):
		return response_success('customer_import')

	def before_customer_import(self, convert, customers, customers_ext):
		return response_success()

	def customer_import(self, convert, customers, customers_ext):
		return response_success(0)

	def after_customer_import(self, customers_id, convert, customers, customers_ext):
		return response_success()

	def addition_customer_import(self, convert, customers, customers_ext):
		return response_success()
	...
	def convert_order_export(self, orders, orders_ext):
		...
		# order_data['tax']['amount'] = get_row_from_list_by_field(orders_ext['orders_products'], 'orders_id', orders['orders_id'])['products_tax']
		order_data['tax']['amount'] = get_value_by_key_in_dict(orders, 'total_tax', 0.0000)
		order_data['shipping']['title'] = 'Shipping'
		order_data['shipping']['amount'] = get_value_by_key_in_dict(orders, 'total_ship', 0.0000)
		order_data['discount']['title'] = 'Discount'
		order_data['discount']['amount'] = 0.0000
		order_data['total']['title'] = 'Total'
		order_data['total']['amount'] = get_value_by_key_in_dict(orders, 'total', 0.0000)
		order_data['subtotal']['title'] = 'Total products'
		order_data['subtotal']['amount'] = get_value_by_key_in_dict(orders, 'total', 0.0000)
		order_data['currency'] = ''
		...
		customer_name = orders['delivery_name']
		# order_customer['first_name'] = orders['ship_fname']
		order_customer['last_name'] = orders['delivery_name']
		...
		# customer_address['first_name'] = orders['ship_fname']
		customer_address['last_name'] = orders['delivery_name']
		...
		customer_address['telephone'] = orders['customers_telephone']
		customer_address['company'] = orders['delivery_company']

		customer_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'countries_name', orders['billing_country'])
		if customer_country:
		...
		# customer_state = get_row_from_list_by_field(orders_ext['data']['zones'], 'code', orders['delivery_state'])
		# if customer_state:
		...
		billing_name = self.get_name_from_string(orders['delivery_name'])
		# order_billing['first_name'] = orders['bill_fname']
		order_billing['last_name'] = orders['delivery_name']
		order_billing['address_1'] = orders['delivery_address1']
		order_billing['address_2'] = orders['delivery_address2']
		order_billing['city'] = orders['billing_city']
		order_billing['postcode'] = orders['billing_postcode']
		order_billing['telephone'] = orders['customers_telephone']
		order_billing['company'] = orders['billing_company']
		# billing_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'alpha', orders['bill_cntry'])
		# if billing_country:
		...
		#
		#
		# billing_state = get_row_from_list_by_field(orders_ext['data']['zones'], 'code', orders[
		# 	'bill_state'])
		# if billing_state:
		...
		delivery_name = self.get_name_from_string(orders['delivery_name'])
		# order_delivery['first_name'] = orders['ship_fname']
		order_delivery['last_name'] = orders['delivery_name']
		...
		order_delivery['telephone'] = orders['customers_telephone']
		order_delivery['company'] = orders['delivery_company']

		# delivery_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'alpha', orders['ship_cntry'])
		# if delivery_country:
		...
		# delivery_state = get_row_from_list_by_field(orders_ext['data']['zones'], 'code', orders['ship_state'])
		# if delivery_state:
		...
		payments = get_row_from_list_by_field(orders['payment_method'], 'orders_id', orders['orders_id'])
		order_payment = self.construct_order_payment()
		order_payment['title'] = 'Payment'
		order_data['payment'] = order_payment

		order_products = get_list_from_list_by_field(orders_ext['data']['orders_products'], 'orders_id', orders['orders_id'])
	...
	def get_order_id_import(self, convert, orders, orders_ext):
		return orders['orders_id']

	def check_order_import(self, convert, orders, orders_ext):
		return True if self.get_map_field_by_src(self.TYPE_ORDER, convert['id'], convert['code']) else False

	def router_order_import(self, convert, orders, orders_ext):
		return response_success('order_import')

	def before_order_import(self, convert, orders, orders_ext):
		return response_success()

	def order_import(self, convert, orders, orders_ext):
		return response_success(0)

	def after_order_import(self, orders_id, convert, orders, orders_ext):
		return response_success()

	def addition_order_import(self, convert, orders, orders_ext):
		return response_success()
	...
	def get_category_parent(self, categories_id):
		query = {
			'type': 'select',
			'query': "SELECT * FROM  _DBPRF_categories WHERE categories_id = " + to_str(categories_id)
		}
