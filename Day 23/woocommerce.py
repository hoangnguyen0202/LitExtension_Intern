...
class LeCartWoocommerce(LeCartWordpress):
	...
	def category_import(self, convert, category, categories_ext):
		...
		self.insert_map(self.insert_map('taxonomy_cat', term_id, term_taxonomy_id))
	...
	def product_import(self, convert, product, products_ext):
		...
		term_taxonomy_id = self.get_map_field_by_src('taxonomy_cat', self.get_map_field_by_src(self.TYPE_CATEGORY, category_id))
		term_relationship = {
			'object_id': ID,
	...
	def customer_import(self, convert, customer, customers_ext):
		...
		# self.insert_map('customer', convert['id'], customer_id)
	...
	def order_import(self, convert, order, orders_ext):
		...
		# order_qty = order_discount = order_tax = order_total = 0
		...
		for orders_product in orders_products:
			...
			# 	'product_net_revenue': float(item['original_price'])*int(item['qty']),
			# 	'product_gross_revenue': float(item['original_price'])*int(item['qty']),
			...
			# self.import_data_connector(order_product_lookup_query, 'order')
		...
		for meta_key in meta_keys:
			...
			# order_discount += float(item['discount_amount'])
			# order_tax += float(item['tax_amount'])
			# order_total = float(item['original_price'])*int(item['qty']) - order_discount - order_tax
		# order_total += float(convert['shipping']['amount'])
		# order_stats = {
		# 	'order_id': order_id,
		# 	'num_items_sold': order_qty,
		# 	'total_sales': order_total,
		# 	'tax_total': order_tax,
		# 	'shipping_total': convert['shipping']['amount'],
		# 	'net_total': order_total + order_discount + order_tax + float(convert['shipping']['amount']),
		# 	'status': 'wc-pending',
		# }
		# order_stats_query = self.create_insert_query_connector('wc_order_stats', order_stats)
		# self.import_data_connector(order_stats_query, 'order')
		...
		# meta_key = ['cost', 'total_tax']
		# meta_value = [convert['shipping']['amount'], order_tax]
		# for i in range(len(meta_key)):
		# 	order_itemmeta = {
		# 		'order_item_id': order_item_id,
		# 		'meta_key': meta_key[i],
