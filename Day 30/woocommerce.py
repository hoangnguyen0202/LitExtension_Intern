...
class LeCartWoocommerce(LeCartWordpress):
	...
	def category_import(self, convert, category, categories_ext):
		...
		category_term['name'] = get_row_from_list_by_field(categories_ext['data']['categories_description'], 'categories_id', convert['id'])['categories_name']
		category_term['slug'] = category_term['name'].casefold().replace(' ', '-')
		...
		term_taxonomy_id = self.import_data_connector(term_taxonomy_query, 'category')
		self.insert_map('taxonomy_category', term_id, term_taxonomy_id)
	...
	def check_product_import(self, convert, product, products_ext):
		product_id = self.get_map_field_by_src(self.TYPE_PRODUCT, convert['id'])
		if product_id:
			attributes = products_ext['data']['attribute']
			for attribute in attributes:
				attribute_labels = self.select_data_connector(self.create_select_query_connector('woocommerce_attribute_taxonomies', '', fields='attribute_label'))
				b = 'false'
				for attribute_label in attribute_labels['data']:
					if attribute['attribute_name'] == attribute_label['attribute_label']: b = 'true'
				if b == 'false':
					attribute_taxonomies = {
						'attribute_name': attribute['attribute_code'],
						'attribute_label': attribute['attribute_name'],
						'attribute_type': 'select',
						'attribute_orderby': 'menu_order',
					}
					attribute_taxonomies_query = self.create_insert_query_connector('woocommerce_attribute_taxonomies', attribute_taxonomies)
					self.import_data_connector(attribute_taxonomies_query, 'product')
			product_attributes = get_list_from_list_by_field(products_ext['data']['products_attribute'], 'product_id', convert['id'])
			value_dict = {}
			for product_attribute in product_attributes:
				terms = self.select_data_connector(self.create_select_query_connector('terms', ''))
				b = 'false'
				for term in terms['data']:
					if product_attribute['value'] == term['name']: b = 'true'
				if b == 'false':
					attribute_terms = {
						'name': product_attribute['value'],
						'slug': product_attribute['value'].replace(' ', '-').lower(),
					}
					attribute_terms_query = self.create_insert_query_connector('terms', attribute_terms)
					term_id = self.import_data_connector(attribute_terms_query, 'product')
					attribute_code = get_row_from_list_by_field(products_ext['data']['attribute'], 'attribute_id', product_attribute['attribute_id'])['attribute_code']
					count = 0
					for prd_attr in product_attributes:
						if prd_attr['value'] == product_attribute['value']: count = 1
					attribute_term_taxonomy = {
						'term_id': term_id,
						'taxonomy': 'pa_' + attribute_code,
						'count': count,
					}
					attribute_term_taxonomy_query = self.create_insert_query_connector('term_taxonomy', attribute_term_taxonomy)
					term_taxonomy_id = self.import_data_connector(attribute_term_taxonomy_query, 'product')
					self.insert_map('taxonomy_attribute', term_id, term_taxonomy_id)
				terms = self.select_data_connector(self.create_select_query_connector('terms', ''))
				for term in terms['data']:
					if product_attribute['value'] == term['name']:
						self.insert_map(self.TYPE_ATTR_VALUE, product_attribute['value_id'], term['term_id'])
				# category_id = get_list_from_list_by_field(products_ext['data']['product_to_category'], 'products_id', product_attribute['product_id'])[0]['categories_id']
				# term_taxonomy_id = self.get_map_field_by_src('taxonomy_category', self.get_map_field_by_src(self.TYPE_CATEGORY, category_id))
				# term_relationship = {
				# 	'object_id': self.get_map_field_by_src(self.TYPE_PRODUCT, product_attribute['product_id']),
				# 	'term_taxonomy_id': term_taxonomy_id,
				# }
				# term_relationship_query = self.create_insert_query_connector('term_relationships', term_relationship)
				# self.import_data_connector(term_relationship_query, 'term_relationship')
				term_relationships = {
					'object_id': self.get_map_field_by_src(self.TYPE_PRODUCT, product_attribute['product_id']),
					'term_taxonomy_id': '2'
				}
				term_relationships_query = self.create_insert_query_connector('term_relationships', term_relationships)
				self.import_data_connector(term_relationships_query, 'product')
				term_relationships = {
					'object_id': self.get_map_field_by_src(self.TYPE_PRODUCT, product_attribute['product_id']),
					'term_taxonomy_id': self.get_map_field_by_src('taxonomy_attribute', self.get_map_field_by_src(self.TYPE_ATTR_VALUE, product_attribute['value_id'])),
				}
				term_relationships_query = self.create_insert_query_connector('term_relationships', term_relationships)
				self.import_data_connector(term_relationships_query, 'product')
				attr = get_row_from_list_by_field(products_ext['data']['attribute'], 'attribute_id', product_attribute['attribute_id'])
				val_dict = {'pa_' + attr['attribute_code']: {
				...
				value_dict.update(val_dict)
			self.log(value_dict, 'value_dict')
			attr_postmeta = {
				'post_id': self.get_map_field_by_src(self.TYPE_PRODUCT, convert['id']),
				'meta_key': '_product_attributes',
				'meta_value': php_serialize(value_dict)
			}
			attr_postmeta_query = self.create_insert_query_connector('postmeta', attr_postmeta)
			self.import_data_connector(attr_postmeta_query, 'product')
	...
	def product_import(self, convert, product, products_ext):
		...
		product_query = self.create_insert_query_connector('posts', product_post)
		ID = self.import_data_connector(product_query, 'product')
		self.insert_map(self.TYPE_PRODUCT, convert['id'], ID)
		meta_key = ['_sku', '_regular_price', '_sale_price', '_price']
		meta_value = [convert['sku'], convert['price'], convert['special_price']['price'], convert['special_price']['price']]
		for i in range(len(meta_key)):
			...
			product_postmeta['post_id'] = ID
			product_postmeta['meta_key'] = meta_key[i]
			product_postmeta['meta_value'] = meta_value[i]
			product_postmeta_query = self.create_insert_query_connector('postmeta', product_postmeta)
			self.import_data_connector(product_postmeta_query, 'product_postmeta')
		category_id = get_list_from_list_by_field(products_ext['data']['product_to_category'], 'products_id', convert['id'])[0]['categories_id']
		term_taxonomy_id = self.get_map_field_by_src('taxonomy_category', self.get_map_field_by_src(self.TYPE_CATEGORY, category_id))
		term_relationship = {
			'object_id': ID,
			'term_taxonomy_id': term_taxonomy_id,
		...
		product_meta_lookup_query = self.create_insert_query_connector('wc_product_meta_lookup', product_meta_lookup)
		self.import_data_connector(product_meta_lookup_query, 'product_meta_lookup')
		image = product['products_image']
		image_process = self.process_image_before_import('http://localhost', 'customcart/images/' + image)
		image_path = self.make_woocommerce_image_path(product['products_image'])
		image_upload = self.uploadImageConnector(image_process, 'wp-content/uploads/' + image_path)
		if type(image_upload) == str:
			guid = 'http://localhost/wordpress/' + image_upload
		else:
			guid = ''
		image_product_post = {
			'post_author': '1',
			'post_title': image.replace('.png', ''),
			'post_status': 'inherit',
			'ping_status': 'closed',
			'post_name': image.replace('.png', '').lower(),
			'post_parent': ID,
			'guid': guid,
			'post_type': 'attachment',
			'post_mime_type': 'image/jpeg'
		}
		image_product_query = self.create_insert_query_connector('posts', image_product_post)
		img_id = self.import_data_connector(image_product_query, 'product')
		image_postmeta = {
			'post_id': img_id,
			'meta_key': '_wp_attached_file',
			'meta_value': image_path,
		}
		image_postmeta_query = self.create_insert_query_connector('postmeta', image_postmeta)
		self.import_data_connector(image_postmeta_query, 'product')
		image_postmeta = {
			'post_id': ID,
	...
	def customer_import(self, convert, customer, customers_ext):
		nickname = customer['customers_firstname'].lower() + customer['customers_lastname'].lower()
		customer_user = {
			'user_login': nickname,
			'user_pass': customer['customers_password'],
			'user_nicename': nickname,
			'user_registered': convert['created_at'],
			'user_email': customer['customers_email_address'],
			'display_name': customer['customers_firstname'] + ' ' + customer['customers_lastname'],
		}
		customer_user_query = self.create_insert_query_connector('users', customer_user)
		user_id = self.import_data_connector(customer_user_query, 'customer')
		company = ''
		address = get_row_from_list_by_field(customers_ext['data']['address_book'], 'customers_id', customer['customers_id'])['entry_street_address']
		city = get_row_from_list_by_field(customers_ext['data']['address_book'], 'customers_id', customer['customers_id'])['entry_city']
		postcode = get_row_from_list_by_field(customers_ext['data']['address_book'], 'customers_id', customer['customers_id'])['entry_postcode']
		# country_id = get_row_from_list_by_field(customers_ext['data']['address_book'], 'customers_id', customer['customers_id'])['entry_country_id']
		# country = get_row_from_list_by_field(customers_ext['data']['countries'], 'countries_id', country_id)['countries_iso_code_2']
		# state_id = get_row_from_list_by_field(customers_ext['data']['address_book'], 'customers_id', customer['customers_id'])['entry_zone_id']
		# state = get_row_from_list_by_field(customers_ext['data']['zones'], 'zone_id', state_id)
		meta_key = ['nickname', 'first_name', 'last_name',
					'billing_first_name', 'billing_last_name', 'billing_company', 'billing_address_1', 'billing_city', 'billing_postcode', 'billing_phone', 'billing_email',
					'shipping_first_name', 'shipping_last_name', 'shipping_company', 'shipping_address_1', 'shipping_city', 'shipping_postcode']
		meta_value = [nickname, customer['customers_firstname'], customer['customers_lastname'],
					  customer['customers_firstname'], customer['customers_lastname'], company, address, city, postcode, customer['customers_telephone'], customer['customers_email_address'],
					  customer['customers_firstname'], customer['customers_lastname'], company, address, city, postcode, ]
		for i in range(len(meta_key)):
			customer_usermeta = {
				'user_id': user_id,
				'meta_key': meta_key[i],
			...
			customer_usermeta_query = self.create_insert_query_connector('usermeta', customer_usermeta)
			self.import_data_connector(customer_usermeta_query, 'customer')
		customer_lookup = {
			'user_id': user_id,
		...
		customer_query = self.create_insert_query_connector('wc_customer_lookup', customer_lookup)
		customer_id = self.import_data_connector(customer_query, 'customer')
	...
	def order_import(self, convert, order, orders_ext):
		...
		order_id = self.import_data_connector(order_post_query, 'order')
		orders_products = get_list_from_list_by_field(orders_ext['data']['orders_products'], 'orders_id', convert['id'])
		for orders_product in orders_products:
			order_items = {
				'order_item_name': orders_product['products_name'],
				'order_item_type': 'line_item',
				'order_id': order_id,
			}
			order_items_query = self.create_insert_query_connector('woocommerce_order_items', order_items)
			order_item_id = self.import_data_connector(order_items_query, 'order')
			qty = orders_product['products_quantity']
			line_subtotal = orders_product['products_price']
			line_total = orders_product['final_price']
			meta_key = ['_qty', '_line_subtotal', '_line_total']
			meta_value = [qty, line_subtotal, line_total]
			for i in range(len(meta_key)):
				order_itemmeta = {
					'order_item_id': order_item_id,
					'meta_key': meta_key[i],
				...
				order_itemmeta_query = self.create_insert_query_connector('woocommerce_order_itemmeta', order_itemmeta)
				self.import_data_connector(order_itemmeta_query, 'order')
		order_shipping = {
			'order_item_name': 'Shipping',
			'order_item_type': 'shipping',
			'order_id': order_id,
		}
		order_shipping_query = self.create_insert_query_connector('woocommerce_order_items', order_shipping)
		order_item_id = self.import_data_connector(order_shipping_query, 'order')
		cost = 0
		order_total = get_list_from_list_by_field(orders_ext['data']['orders_total'], 'orders_id', convert['id'])
		ot_shipping = get_row_from_list_by_field(order_total, 'class', 'ot_shipping')
		if ot_shipping: cost = ot_shipping['value']
		order_itemmeta = {
			'order_item_id': order_item_id,
			'meta_key': 'cost',
			'meta_value': cost,
		...
		order_tax_query = self.create_insert_query_connector('woocommerce_order_items', order_tax)
		order_item_id = self.import_data_connector(order_tax_query, 'order')
		fee = 0
		order_total = get_list_from_list_by_field(orders_ext['data']['orders_total'], 'orders_id', convert['id'])
		ot_tax = get_row_from_list_by_field(order_total, 'class', 'ot_tax')
		if ot_tax: fee = ot_tax['value']
		meta_keys = ['_fee_amount', '_line_total']
		...
		meta_key = ['_billing_first_name', '_billing_last_name', '_billing_company', '_billing_city', '_billing_state', '_billing_postcode', '_billing_country', '_billing_phone', '_billing_address_1', '_billing_email',
					'_shipping_first_name', '_shipping_last_name', '_shipping_company', '_shipping_city', '_shipping_state', '_shipping_postcode', '_shipping_country', '_shipping_address_1',
					'_order_shipping', '_order_total']
		meta_value = [convert['billing_address']['first_name'], convert['billing_address']['last_name'], convert['billing_address']['company'], convert['billing_address']['city'], convert['billing_address']['state']['name'], convert['billing_address']['postcode'], convert['billing_address']['country']['name'], convert['billing_address']['telephone'], convert['billing_address']['address_1'], convert['customer']['email'],
					  convert['shipping_address']['first_name'], convert['shipping_address']['last_name'], convert['shipping_address']['company'], convert['shipping_address']['city'], convert['shipping_address']['state']['name'], convert['shipping_address']['postcode'], convert['shipping_address']['country']['name'], convert['shipping_address']['address_1'],
					  cost, ot_total['value']]
		for i in range(len(meta_key)):
			order_postmeta = {
				'post_id': '',
			...
			order_postmeta['post_id'] = order_id
			order_postmeta['meta_key'] = meta_key[i]
			order_postmeta['meta_value'] = meta_value[i]
			order_postmeta_query = self.create_insert_query_connector('postmeta', order_postmeta)
			self.import_data_connector(order_postmeta_query, 'order')
